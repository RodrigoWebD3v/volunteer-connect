...
export async function load({ locals, params, url }) {
	try {
		...
		return {
			oportunidade: await backendRequest<Record<string, unknown>>(
				locals,
				`/oportunidades/minhas/${params.id}`
			),
			bloqueio: null
		};
	} catch (error) {
		if (error instanceof BackendHttpError) {
			return { oportunidade: null, bloqueio: error.message };
		}
...
export const actions = {
	default: async ({ locals, params, request }) => {
		...
		try {
			await backendRequest(locals, `/oportunidades/${params.id}`, {
				method: 'PATCH',
			...
			return fail(500, { ...valores, erro: 'Nao foi possivel salvar a oportunidade.' });
		}
